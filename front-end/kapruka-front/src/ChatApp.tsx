...
export default function ChatApp() {
    ...
    const toggleSpeak = (text: string, index: number) => {
        ...
        // Strip markdown and clean text for synthesis
                        ...
                        {messages.length === 1 ? (
                            ...
                            >
                                {messages.map((msg, index) => (
                                    ...
                                    {msg.timestamp && (
                                        <div className="flex justify-between items-center gap-1 mt-1 text-[10px] font-medium opacity-70 w-full">
                                            {/* Speak button for AI bot responses */}
                                            {msg.role !== 'user' && msg.text && (
                                                <button 
                                                    onClick={() => toggleSpeak(msg.text, index)}
                                                    className="p-1 rounded-full hover:bg-white/10 transition-colors flex items-center justify-center cursor-pointer -ml-1 text-current"
                                                    title={speakingIndex === index ? "Stop reading" : "Read aloud"}
                                                >
                                                    {speakingIndex === index ? (
                                                        <svg className="w-3.5 h-3.5 animate-pulse text-pink-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                        ...
                        >
                            {suggestedActions.map((action, i) => (
                                <motion.button
                                    ...
                                    className={`px-3.5 py-2 rounded-full text-[11px] font-semibold transition-all duration-200 hover:scale-105 active:scale-95 cursor-pointer gum-control pulse-glow-chip ${
                                        darkMode
